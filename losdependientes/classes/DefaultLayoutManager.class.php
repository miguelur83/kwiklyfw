<?php
  class DefaultLayoutManager{
    ...
    public function getCreateForm($action_URL){
      ...
      $form_html = "<p>Ingrese la informaci&oacute;n para un nuevo ".get_class($this->my_object)." y presione Crear. (*) Campos obligatorios.</p>";
      ...
      $form_html .= "<input type='submit' id='frm_create_".get_class($this->my_object)."_submit' name='frm_create_".get_class($this->my_object)."_submit' value='Crear' />";
      $form_html .= "&nbsp;o <a href='javascript:javascript:history.go(-1)'>Cancelar</a>";
      $form_html .= "</form>";
      return $form_html;
    }
    
    public function getUpdateForm($action_URL){
      echo "<p>Editando ".get_class($this->my_object)." #".$this->my_object->getField("id")."...</p>"; 
      $fields = $this->my_object->getField('fields');
      $editable = $this->my_object->getField('editable');
      $mandatory = $this->my_object->getField('mandatory');
      $field_types = $this->my_object->getField('field_types');
      
      $form_html = "<p>Actualice la informaci&oacute;n para ".get_class($this->my_object)." y presione Actualizar. (*) Campos obligatorios.</p>";
      ...
      $form_html .= "<input type='submit' id='frm_update_".get_class($this->my_object)."_submit' name='frm_update_".get_class($this->my_object)."_submit' value='Actualizar' />";
      $form_html .= "&nbsp;o <a href='javascript:javascript:history.go(-1)'>Cancelar</a>";
      $form_html .= "</form>";
      return $form_html;
    }
    ...
    public function getDeleteForm($action_URL){            
      ...
      $form_html = "<p>Est&aacute;s por eliminar ".get_class($this->my_object)." #".$this->my_object->getField("id").". Seguro de que quer&eacute;s continuar?</p>";
      ...
      $form_html .= "<input type='submit' id='frm_delete_".get_class($this->my_object)."_submit' name='frm_delete_".get_class($this->my_object)."_submit' value='Borrar' />";
      $form_html .= "&nbsp;o <a href='javascript:javascript:history.go(-1)'>Cancelar</a>";
      $form_html .= "</form>";
      return $form_html;
    }
    ...
    public function getViewableList($pageURL, $sorted_by = "id", $desc = 0, $filter=''){
	  ...
	  $list_html = '';
      $list_html .= "<a class='add_link' href='".$pageURL."&action=create'>Cargar ".$this->getTitleFromName(get_class($this->my_object))."</a>";
      
	  if ($this->my_object->getFilterField() != ''){
	  	$list_html .= "<form id='filter' name='filter' method='post' action='".$pageURL."'><input type='text' name='filtertxt' id='filtertxt' value='".$_GET['filter']."' /><input type='submit' id='filter_submit' name='filter_submit' value='Filtrar por ".ucwords($this->my_object->getFilterField())."' /></form>";
	  }
	  
	  $list_html .= '<div class="pagination">Mostrando '.$page_from.'-'.$page_to.' de '.$resultados.'&nbsp;<a href="'.($prev_page?$pageURL.'&sortby='.$sorted_by.'&p='.$prev_page:'#').(($_GET['filter'])?"&filter=".$_GET['filter']:"").'">&laquo;</a>&nbsp;Pag '.$page.' de '.$total_pages.'&nbsp;<a href="'.($next_page?$pageURL.'&sortby='.$sorted_by.'&p='.$next_page:'#').(($_GET['filter'])?"&filter=".$_GET['filter']:"").'">&raquo;</a></div>';
      $list_html .= "<table><tr>";
      ...
      $list_html .= "<th colspan='2'>Acciones</th>";
      $list_html .= "</tr>";
      ...
      foreach ($all_items as $an_item){
        ...
        foreach ($fields as $field){
          switch ($field_types[$field]){
            case 'collection':
              $list_html .= "<td>(Colecci&oacute;n)</td>";
            break;
            case 'persona':                        
            ...
            case 'imagen_noticia':
				switch ($field_types[$field]){
					...
					case 'imagen_noticia': $folder = 'imagen_noticia/full/'; break;
				}
				if ($an_item[$field] != ''){
            ...
            break;
            case 'boolean':
              $list_html .= "<td>".(($an_item[$field] == '1')?"S&iacute;":"No")."</td>";
            break;
            case 'tipo_critica':
            ...
            break;
            case 'pais':
			  $pais = new Pais($an_item[$field]);
              $list_html .= "<td>".$pais->getField('nombre')."</td>";
            break;
            case 'order':
        ...
        $list_html .= "<td><a class='edit_link' title='Editar' href='".$pageURL."&action=edit&id=".$an_item['id']."'>Editar</a></td><td><a class='delete_link' title='Borrar' href='".$pageURL."&action=delete&id=".$an_item['id']."'>Borrar</a></td>";
        $list_html .= "</tr>";
      }
      $list_html .= "</table>";
      return $list_html;
    ...
    public function getInputForField($field_name, $field_type, $required, $locked = 0, $prefix = '', $hide_field = 0){
      switch ($field_type){
        ...
        case 'pais':
          $input_html  = "<label for='".$prefix.$field_name."'>".$this->getTitleFromName($field_name).(($required)?" (*)":"").":</label> ";
          $pais = new Pais($this->my_object->getField($field_name));
          $input_html .= "<input ".($locked?"disabled='disabled' ":"")." type='text' class='autocomplete_field' id='".$prefix.$field_name."' name='".$prefix.$field_name."' value='".$pais->getField("nombre")."' />";
          $field_help = $this->my_object->getField('field_help');
          if ($field_help[$field_name]){
            $input_html .= "<br /><span class='help_text'>".$field_help[$field_name]."</span>";
          }
        ...
        case 'boolean':   
          $input_html  = "<label for='".$prefix.$field_name."'>".$this->getTitleFromName($field_name).(($required)?" (*)":"").":</label> ";
          $input_html .= "<input ".($locked?"disabled='disabled' ":"")." type='radio' id='".$prefix.$field_name."_critica' name='".$prefix.$field_name."' value='1'".($this->my_object->getField($field_name)?" checked='checked'":"")." /> S&iacute;";
        ...
        case 'richtext':
          ...
          $input_html .= "<br /><textarea style='width:100%;height:400px;' ".($locked?"disabled='disabled' ":"")." id='".$prefix.$field_name."' name='".$prefix.$field_name."'>".$this->my_object->getField($field_name)."</textarea><br /><br />";
        break;
		case 'foto_critica':
		case 'banner_home':
		case 'logo_home':
		...
		case 'imagen_noticia':
			switch ($field_type){
				...
				case 'afiche_pelicula': $folder = 'afiche_pelicula/home/'; break;
				case 'tapa_libro': $folder = 'tapa_libro/home/'; break;
				case 'tapa_libro': $folder = 'imagen_noticia/full/'; break;
			}
          $input_html  = "<label for='".$prefix.$field_name."'>".$this->getTitleFromName($field_name).(($required)?" (*)":"").":</label> ";
        ...
        case 'pdf':
          $input_html  = "<label for='".$prefix.$field_name."'>".$this->getTitleFromName($field_name).(($required)?" (*)":"").":</label> ";
          $input_html .= "<input type='hidden' name='MAX_FILE_SIZE' value='52428800' />";
          $file_name = $this->my_object->getField($field_name);
          if ($file_name != ""){
        ...
        case 'date':
          $input_html  = "<label for='".$prefix.$field_name."'>".$this->getTitleFromName($field_name).(($required)?" (*)":"").":</label> ";
          $input_html .= "<input class='datepicker' ".($locked?"disabled='disabled' ":"")."type='text' id='".$prefix.$field_name."' name='".$prefix.$field_name."' value='".$this->my_object->getField($field_name)."' maxlength='255' />";
          $field_help = $this->my_object->getField('field_help');
          if ($field_help[$field_name]){
            $input_html .= "<br /><span class='help_text'>".$field_help[$field_name]."</span>";
          }
