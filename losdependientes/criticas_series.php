<?php
  ...
  $criticas_count = $GLOBALS['db']->execute('SELECT COUNT(`criticas`.id) as criticas_count FROM `criticas` INNER JOIN `peliculas` ON `criticas`.pelicula = `peliculas`.id INNER JOIN personas ON criticas.critico = personas.id WHERE `peliculas`.tipo = 1 AND publicar=1 ORDER BY '.$order_by);
  ...
  $criticas_results = $GLOBALS['db']->execute('SELECT `criticas`.id as critica, peliculas.id as pelicula FROM `criticas` INNER JOIN `peliculas` ON `criticas`.pelicula = `peliculas`.id INNER JOIN personas ON criticas.critico = personas.id WHERE `peliculas`.tipo = 1 AND publicar=1 ORDER BY '.$order_by.' LIMIT '.$offset.', '.$limit);
?>
<div id="centro" class="criticas_cine">
      <div id="criticas_cine">
        <div class="barra_titulo">
          <span></span>
          <h1>Cr&iacute;ticas de Series</h1>
