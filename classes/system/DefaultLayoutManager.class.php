<?php
  class DefaultLayoutManager{
    ...
    public function getCreateForm($action_URL){
      ...
      $form_html .= "<form enctype='multipart/form-data' id='frm_create_".get_class($this->my_object)."' name='frm_create_".get_class($this->my_object)."' method='post' action='".$action_URL."'>";
		...
		if($_GET['componente_id']){
			$form_html .= "<input type='hidden' id='componente' name='componente' value='".$_GET['componente_id']."' />";
		}
		if($this->my_object->getField('componente')){
			$form_html .= "<input type='hidden' id='componente' name='componente' value='".$this->my_object->getField('componente')."' />";
		}
    ...
    public function getViewableList($pageURL, $sorted_by = '', $desc = 0, $filter='', $collector=null){
		if ($sorted_by == ''){
	  ...
	  if (! is_null($collector)){
		$cond .= " AND ".$this->my_object->getField('collector_field')."=".$collector->getField('id');
	  }
	  ...
	  $list_html = '';
      $list_html .= "<a class='add_link' href='".$pageURL."&action=create".((! is_null($collector))?"&collector=".get_class($collector)."&collector_id=".$collector->getField('id'):"")."'>".$GLOBALS['lang']->getString('lnk_create_new_object', $this->getTitleFromName(get_class($this->my_object)))."</a>";
      ...
      foreach ($fields as $field){
	  	if (($field != 'ordernum') && ($field != 'componente') && ($field != $this->my_object->getField('collector_field'))){
      ...
      foreach ($all_items as $an_item){
        ...
        foreach ($fields as $field){
          switch ($field_types[$field]){
            ...
            break;        
            case 'options':        
				$options=$this->my_object->getField('options');
              $list_html .= "<td>".$options[$field][$an_item[$field]]."</td>";
            break;      
            case 'genero':                      
              $my_object_class = get_class($this->my_object);
              $this->my_object = new $my_object_class($an_item['id']);
            ...
            case 'banner_titulo':
				switch ($field_types[$field]){
					...
					case 'imagen_marquesina': $folder = 'imagen_marquesina/'; break;
					case 'banner_home': $folder = 'banner_home/'; break;
					case 'banner_titulo': $folder = 'banner_titulo/'; break;
				}
				if ($an_item[$field] != ''){
            ...
            break;
            case 'text':
              $list_html .= "<td>".stripslashes(htmlentities(substr($an_item[$field],0,100)))."</td>";
            break;
            case 'boolean':
        ...
        $list_html .= "<td><a class='edit_link' title='".$GLOBALS['lang']->getString('lbl_edit')."' href='".$pageURL."&action=edit&id=".$an_item['id'].((! is_null($collector))?"&collector=".get_class($collector)."&collector_id=".$collector->getField('id'):"")."'>".$GLOBALS['lang']->getString('lbl_edit')."</a></td><td><a class='delete_link' title='".$GLOBALS['lang']->getString('lbl_delete')."' href='".$pageURL."&action=delete&id=".$an_item['id'].((! is_null($collector))?"&collector=".get_class($collector)."&collector_id=".$collector->getField('id'):"")."'>".$GLOBALS['lang']->getString('lbl_delete')."</a></td>";
        $list_html .= "</tr>";
      }
      $list_html .= "</table>";
      return $list_html;
      ...
      switch ($field_type){
		...
		case 'options':
			...
			foreach($options as $key => $option){
				$input_html .= "<option ".(($this->my_object->getField($field_name)==$key)?"selected='selected' ":"")."value='".$key."'>".$option."</option>";
			}
        ...
        case 'boolean':   
          $input_html  = "<label for='".$prefix.$field_name."'>".$this->getTitleFromName($field_name).(($required)?" (*)":"").":</label> ";
          $input_html .= "<input ".($locked?"disabled='disabled' ":"")." type='radio' id='".$prefix.$field_name."_critica' name='".$prefix.$field_name."' value='1'".($this->my_object->getField($field_name)?" checked='checked'":"")." /> ".$GLOBALS['lang']->getString('txt_bool_yes');
          $input_html .= "&nbsp;<input ".($locked?"disabled='disabled' ":"")." type='radio' id='".$prefix.$field_name."_resenia' name='".$prefix.$field_name."' value='0'".(!$this->my_object->getField($field_name)?" checked='checked'":"")." /> ".$GLOBALS['lang']->getString('txt_bool_no');
          $field_help = $this->my_object->getField('field_help');
          if ($field_help[$field_name]){
            $input_html .= "<br /><span class='help_text'>".$field_help[$field_name]."</span>";
          }
        ...
        case 'text':
          ...
          $input_html .= "<br /><textarea class='mceNoEditor' style='width:100%;height:200px;' ".($locked?"disabled='disabled' ":"")." id='".$prefix.$field_name."' name='".$prefix.$field_name."'>".stripslashes($this->my_object->getField($field_name))."</textarea><br /><br />";
        break;
		case 'foto_critica':
		case 'logo_home':
		case 'afiche_pelicula':
		...
		case 'collection':
          $input_html  = "<label for='".$prefix.$field_name."'>".$this->getTitleFromName($field_name).(($required)?" (*)":"").":</label> ";
			$collections = $this->my_object->getField("collections");
			$collection_class = $collections[$field_name]['class_name'];
			$an_object = new $collection_class();
          $input_html .= "<br />";
			$input_html .= $an_object->getLayoutManager()->getViewableList($an_object->getField('manage_url'), '', 0, '', $this->my_object);
          $input_html .= "<br /><br />";
		break;
        case 'string':
        default:
